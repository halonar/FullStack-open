...
test("at start url, likes and username are not displayed", () => {
  const blogs = [];
  const setBlogs = () => {};
...
test("after clicking the button, likes and username are displayed", async () => {
  ...
  const fullView = screen.queryByText(
    "Component testing is done with react-testing-library John Doe jd.fi 10 mluukkai"
  );
...
test("when like-button is clicked twice, component's input props event handler function is called twice.", async () => {
  ...
  const fullView = screen.queryByText(
    "Component testing is done with react-testing-library John Doe jd.fi 12 mluukkai"
  );
